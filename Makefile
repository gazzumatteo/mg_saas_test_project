...
dbshell: ## Get a Database shell
	@docker compose exec db psql -U postgres mg_saas_test_project
...
gcp-deploy:  ## Deploy the latest docker container to Google Cloud
	@gcloud run deploy mg-saas-test-project-web \
	--region ${REGION} \
	--update-env-vars DJANGO_SETTINGS_MODULE=mg_saas_test_project.settings_production \
	--image ${IMAGE_URL} \
